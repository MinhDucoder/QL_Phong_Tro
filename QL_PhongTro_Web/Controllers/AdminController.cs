...

{
    public class AdminController : Controller
    {
        ...
        {
            // Bắt đầu với danh sách dien và bao gồm thông tin phòng
            var dienQuery = _ql.Diens.Include(d => d.MaPhongNavigation).AsQueryable();

            // Kiểm tra xem searchString có tồn tại không
            if (!string.IsNullOrEmpty(searchString))
            {
                // Lấy danh sách mã phòng phù hợp với số phòng tìm kiếm
                var maList = _ql.Phongs
                    .Where(p => p.SoPhong.Contains(searchString))
                    .Select(p => p.MaPhong)
                    .ToList();

                // Lọc danh sách dien theo mã phòng
                dienQuery = dienQuery.Where(p => p.MaPhong.HasValue && maList.Contains(p.MaPhong.Value));
            }
            ...
            // Trả về view với danh sách dien đã phân trang
            return View("Dien", dienQuery.OrderBy(p => p.MaPhong).ToPagedList(pageNumber, pageSize));
        }

        public IActionResult SearchNuoc(string searchString, int? page)
        {
            // Bắt đầu với danh sách nuoc và bao gồm thông tin phòng
            var nuocQuery = _ql.Nuocs.Include(n => n.MaPhongNavigation).AsQueryable();
            ...
            if (!string.IsNullOrEmpty(searchString))
            {
                ...
                // Lọc danh sách nuoc theo mã phòng
                nuocQuery = nuocQuery.Where(p => p.MaPhong.HasValue && maList.Contains(p.MaPhong.Value));
            }
            ...
            // Trả về view với danh sách nuoc đã phân trang
            return View("Nuoc", nuocQuery.OrderBy(p => p.MaPhong).ToPagedList(pageNumber, pageSize));
        }

        public IActionResult SearchHD(string searchString, int? page)
        {
            // Sử dụng Include để nạp dữ liệu liên quan
            var hdQuery = _ql.HoaDons.Include(hd => hd.MaPhongNavigation) // Nạp MaPhongNavigation
                                       .AsQueryable();

            // Lọc theo số phòng nếu searchString không rỗng
            if (!string.IsNullOrEmpty(searchString))
            {
                hdQuery = hdQuery.Where(hd => hd.MaPhongNavigation != null &&
                                               hd.MaPhongNavigation.SoPhong.Contains(searchString));
            }

            int pageSize = 8;
            int pageNumber = page ?? 1;

            // Trả về view với danh sách hóa đơn đã phân trang
            return View("DH", hdQuery.OrderBy(hd => hd.MaHoaDon).ToPagedList(pageNumber, pageSize));
        }
        public IActionResult SearchTen(string searchString, int? page)
        {
            // Khởi tạo truy vấn với danh sách khách thuê
            var khQuery = _ql.KhachThues.Include(hd => hd.MaPhongNavigation).AsQueryable();

            // Kiểm tra xem searchString có tồn tại không
            if (!string.IsNullOrEmpty(searchString))
            {
                // Lọc danh sách khách thuê theo tên khách hàng
                khQuery = khQuery.Where(kh => kh.Ten.Contains(searchString));
            }
            ...
            // Trả về view với danh sách khách thuê đã phân trang
            return View("KH", khQuery.OrderBy(kh => kh.MaKhachThue).ToPagedList(pageNumber, pageSize));
        }
        ...
        {
            var tk = from p in _ql.TaiKhoans select p;
        ...
        public IActionResult Dien(int? page, string success)
        {
            ...
            int pageNumber = page ?? 1;
            var dien = _ql.Diens
                      .Include(d => d.MaPhongNavigation) // Bao gồm thông tin bảng Phong
                      .OrderBy(p => p.MaDien)
        ...
        public async Task<IActionResult> KH(int? page, string success)
        {
            ...
            int pageSize = 8;
            int pageNumber = page ?? 1;
            var khachThues = _ql.KhachThues
                       .Include(k => k.MaPhongNavigation) // Bao gồm thông tin bảng Phong
                       .OrderByDescending(p => p.MaKhachThue)
                       .ToPagedList(pageNumber, pageSize);
        ...
        public async Task<IActionResult> DH(int? page)
        {
            int pageSize = 8;
            int pageNumber = page ?? 1;
            var hoaDons = _ql.HoaDons
                    .Include(h => h.MaPhongNavigation) // Bao gồm thông tin từ bảng Phong
                    .OrderBy(p => p.MaHoaDon)
                    .ToPagedList(pageNumber, pageSize);
            return View(hoaDons);
        }
